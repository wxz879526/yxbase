...
public:
	...
	{
		base::ScopedFILE file(base::OpenFile(db_path(), type == TYPE_OVERRIDE_AND_TRUNCATE
			? "wb" : "rb+"));
		fseek(file.get(), 0, SEEK_SET);
		const char *kJunk = "Now is the winner";
...
class CSqlite3Demo : public SQLTestBase
{
public:
	CSqlite3Demo()
	{
		auto b = db().Execute("CREATE TABLE user(a, b)");
		auto bRet= db().Execute("INSERT INTO user (a, b) VALUES(1, '250')");
		int x = 0;
	}

	int RecordRows()
	{
		sql::Statement stat(db().GetUniqueStatement("SELECT count(*) FROM user"));
		stat.Step();
		return stat.ColumnInt(0);
	}
};